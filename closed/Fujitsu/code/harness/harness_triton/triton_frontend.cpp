                      ...
                      size_t buffer_manager_thread_count, bool batch_triton_requests)
{
    ...
    // FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetLogVerbose(server_options, 1), "Setting verbose level 1");
    ...
    size_t pool_item_count =
        2 * instance_count * (m_MaxBatchSize / min_sample_size + 1) * output_names.size();
    ...
    LOG(INFO) << "Allocated Pinned memory pool of " << pool_item_count * pool_item_size << " bytes for single query requests.";
    // For batched requests, we need 2 sets of output buffers, one of them is used for the 
    // batched requests and the other for single-sample requests. The size and numbers of these
    ...
    if(m_BatchTritonRequests)
    {
        ...
        LOG(INFO) << "Allocated Pinned memory pool of count: " << pool_item_count_batch <<" size :" << pool_item_size_batch << " bytes for batched requests.";
    }
...
bool Server_SUT::CheckContiguity(const std::vector<mlperf::QuerySample>& samples, int start_id,
                                 int end_id)
{
    ...
    {
        auto prev =
            static_cast<int8_t*>(m_SampleLibrary->GetSampleAddress(samples[start_id].index, i));
        ...
        {
            auto next =
                static_cast<int8_t*>(m_SampleLibrary->GetSampleAddress(samples[it].index, i));
...
bool Server_SUT::CheckDLRMContiguity(const std::vector<mlperf::QuerySample>& samples, int start_id,
                                     int end_id, int num_pairs)
{
    bool contiguous = true;
    auto qsl = reinterpret_cast<DLRMSampleLibrary*>(m_SampleLibrary.get());
    ...
    for(size_t i = 0; i < m_InputTensors.size() && contiguous; i++)
    {
        int8_t* first_task_start =
            static_cast<int8_t*>(qsl->GetSampleAddress(samples[start_id].index, i));

        int8_t* last_task_start =
            static_cast<int8_t*>(qsl->GetSampleAddress(samples[end_id - 1].index, i));
                                               ...
                                               const std::vector<size_t> batchNumPairs)
{
    ...
    request_block->m_BatchResponseMetadata.dlrmNumPairsList = batchNumPairs;
    ...
    request_block->m_ResponseMetadata.m_PaddingSize =
        (num_user_item_pairs % 2) ? m_OutputPaddingSize : 0;
...
void Server_SUT::IssueTritonContiguousBatch(const std::vector<mlperf::QuerySample>& samples,
                                 ...
                                 const BatchResponseMetaData* response_metadata)
{
    ...
    {
        int numQueryResponses = response_metadata->dlrmNumPairsList.size();
        auto buffer_ptr = static_cast<const int8_t*>(output0_content);
        ...
        for(int i = 0; i < numQueryResponses; i++)
        {
            loadgen_responses.emplace_back(
                mlperf::QuerySampleResponse{(response_metadata->m_ResponseId)[i], output0_result,
                                            response_metadata->dlrmNumPairsList[i]*TRITONSERVER_DataTypeByteSize(datatype)});
            cumulativeNumPairs += response_metadata->dlrmNumPairsList[i];
