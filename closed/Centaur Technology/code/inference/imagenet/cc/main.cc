...
ABSL_FLAG(std::vector<std::string>, outdir, {"/tmp/mlperf"}, "Output directory for Loadgen.");
// Batching settings.
...
int main(int argc, char** argv) {
  ...
  // Bazel doesn't like passing escaped space characters as args, which could
  // definitely exist in the output path here. So we'll accommodate passing a
  // path with commas instead of spaces, and exchange them back here.
  bool first = true;
  std::string outdir = "";
  for(std::string s : absl::GetFlag(FLAGS_outdir)) {
    if(!first) {
      outdir += " ";
    }
    outdir += s;
  ...
  mlperf::Option option(
      ...
      outdir, absl::GetFlag(FLAGS_export_model_path), absl::GetFlag(FLAGS_model_name),
