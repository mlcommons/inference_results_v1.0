...
struct BatchResponseMetaData
{
    ...
    BatchResponseMetaData(Server_SUT* server_ptr)
        : m_ServerPtr(server_ptr), m_TracePtr(nullptr), m_PaddingSize(0)
    {
    }
    Server_SUT* m_ServerPtr;
    ...
    std::vector<size_t> dlrmNumPairsList;
    TRITONSERVER_InferenceTrace* m_TracePtr;
    // FIXME assuming that there is only one output
    size_t m_PaddingSize;
};
...
class Server_SUT : public mlperf::SystemUnderTest
{
  public:
    ...
    void Init(size_t min_sample_size = 1, size_t max_sample_size = 1, size_t buffer_manager_thread_count = 0, bool batch_triton_requests = false);
    void ModelMetadata();
    void ModelStats();
  ...
  private:
    ...
    void IssueTritonContiguousBatch(const std::vector<mlperf::QuerySample>& samples,
                                size_t start_idx, size_t end_idx);   
    void IssueContiguousBatchDLRMQuery(const std::vector<mlperf::QuerySample>& samples,
                                            size_t batch_start_sample, size_t batch_end_sample, size_t num_user_item_pairs, const std::vector<size_t> batchNumPairs);
