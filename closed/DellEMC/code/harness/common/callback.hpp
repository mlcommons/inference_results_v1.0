...
// Call back for TFRN50 - need to shift detected classes by 1 due to extra background class in the TF model.
void tfrn50Callback(::mlperf::QuerySampleResponse* responses, std::vector<::mlperf::QuerySampleIndex> &sample_ids, size_t response_count)
{
    ...
    for (size_t i = 0; i < response_count; i++)
    {
        auto &r = responses[i];
        size_t size = r.size;
        int32_t modified = *(reinterpret_cast<int32_t*>(r.data)) - offset;
        *(reinterpret_cast<int32_t*>(r.data)) = modified;
    }
}
