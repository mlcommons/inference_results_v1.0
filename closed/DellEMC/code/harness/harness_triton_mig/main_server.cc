    ...
    triton_frontend::sharedBAMap_ptr s2c_baddr_map)
{
    ...
    auto sutshim = std::make_shared<triton_frontend::SUTShim>("Triton_MultiMigServer", 
        scenario, FLAGS_batch_size, FLAGS_deque_timeout_us, mig_uuids,
        c2s_state_mq_map, s2c_state_mq_map, c2s_req_mq_map, s2c_resp_mq_map, c2s_shmem_map, s2c_shmem_map);
    ...
    if (FLAGS_mig_uuid == "N/A")
    {
        ...
        for (auto& u_ : mig_uuids)
        {
            ...
            if (c_pid == 0)
            {
                ...
                if (gpu_numa_map.size() > 1)
                {
                    ...
                    int set_cpu_affnity_res = sched_setaffinity(c_pid, sizeof(mask), &mask);
        ...
        for (auto& u_ : mig_uuids)
        {
            ...
            try
            {
                ...
                assert(rcvd && m_ == triton_frontend::IPCcomm::Bootup);
            }
            catch (bip::interprocess_exception& ex)
            {
