...
PinnedMemoryPool::PinnedMemoryPool(const size_t element_count, const size_t element_byte_size,
    ...
    : m_Head(nullptr), m_Blocks(element_count), m_Buffer(nullptr), m_s2c_shmem(s2c_shmem)
{
    assert(element_count * element_byte_size <= m_s2c_shmem->get_size());

    // No change for CPU alloc?
                      ...
                      size_t buffer_manager_thread_count)
{
    ...
    // FIXME: CPU - FORCE TF VERSION 2, + other settings required to run TF2 in Triton.
    FAIL_IF_ERR(
        TRITONSERVER_ServerOptionsSetBackendConfig(server_options, "tensorflow", "version", "2"),
        "setting default Tensorflow version to TF2");
    ...
    // FIXME: proper instance count for CPU?
        ...
        if(rcvd)
        {
            switch(m_)
            {
                ...
                case IPCcomm::IssueQuery:
                {
                    ...
                    {
                        auto a1 = std::async(std::launch::async, &Server_SUT::IssueQueryInternal,
                                             this, samples, 0, samples.size() / 2);
                        auto a2 = std::async(std::launch::async, &Server_SUT::IssueQueryInternal,
                                             this, samples, samples.size() / 2, samples.size());
                        a1.get();
                        a2.get();
                    }
                    break;
                ...
                {
                    sample_ids.clear();
                    QuerySampleIndex_vec* QSI_vec_data =
                        m_c2s_shmem->find<QuerySampleIndex_vec>("QSIL").first;
                ...
                case IPCcomm::IssueUnload:
                {
                    sample_ids.clear();
                    QuerySampleIndex_vec* QSI_vec_data =
                        m_c2s_shmem->find<QuerySampleIndex_vec>("QSIU").first;
                    for(auto& s_ : *QSI_vec_data)
                    {
                        sample_ids.push_back(s_);
                    }
...
void SUTShim::Init()
{
    ...
    if(m_scenario == "server")
    {
        ...
        for(auto& u_ : m_mig_uuids)
        {
            ...
            m_SQ_handlers[u_] = std::thread(&SUTShim::ServerQueueHandler, this, u_,
                                            m_max_server_buffer_size, m_server_queue_timeout);
        }
    }
}
...
void SUTShim::ServerQueueHandler(std::string mig_uuid, uint64_t max_sq_size,
        ...
        if(!samples.empty())
        {
            try
            {
                ...
                QuerySample_vec* QS_vec_data =
                    m_c2s_shmem_map[mig_uuid]->construct<QuerySample_vec>("QS")(alloc_QS);
                for(auto& s_ : samples)
            ...
            {
                std::cerr << "Met exception while sending Samples/IssueQuery to " << mig_uuid
                          << " : " << ex.what() << std::endl;
                assert(false);
            }
        }
...
void SUTShim::Do_LoadSamplesToRam(std::vector<mlperf::QuerySampleIndex>& samples)
{
    ...
    for(auto& u_ : m_mig_uuids)
    {
        try
        {
            ...
            QuerySampleIndex_vec* QSI_vec_data =
                m_c2s_shmem_map[u_]->construct<QuerySampleIndex_vec>("QSIL")(alloc_QS);
            for(auto& s_ : samples)
        ...
        {
            std::cerr << "Met exception while sending Samples/LoadSamples to " << u_ << " : "
                      << ex.what() << std::endl;
            assert(false);
        }
    }
    ...
    for(auto& u_ : m_mig_uuids)
    {
        ...
        {
            std::cerr << "Met exception while waiting LoadDone from " << u_ << " : " << ex.what()
                      << std::endl;
            assert(false);
        }
    }
...
void SUTShim::Do_UnloadSamplesFromRam(std::vector<mlperf::QuerySampleIndex>& samples)
{
    ...
    for(auto& u_ : m_mig_uuids)
    {
        try
        {
            ...
            const QuerySampleIndex_alloc alloc_QS(m_c2s_shmem_map[u_]->get_segment_manager());
            QuerySampleIndex_vec* QSI_vec_data =
                m_c2s_shmem_map[u_]->construct<QuerySampleIndex_vec>("QSIU")(alloc_QS);
            for(auto& s_ : samples)
                QSI_vec_data->push_back(s_);
        ...
        {
            std::cerr << "Met exception while sending Samples/UnloadSamples to " << u_ << " : "
                      << ex.what() << std::endl;
            assert(false);
        }
    }
    ...
    for(auto& u_ : m_mig_uuids)
    {
        ...
        {
            std::cerr << "Met exception while waiting UnloadDone from " << u_ << " : " << ex.what()
                      << std::endl;
            assert(false);
        }
    }
...
void SUTShim::IssueQuery(const std::vector<mlperf::QuerySample>& samples)
{
        ...
        for(auto& u_ : m_mig_uuids)
        {
            if(samples_map[u_]->size() > 0)
            {
                try
                {
                    ...
                    QuerySample_vec* QS_vec_data =
                        m_c2s_shmem_map[u_]->find_or_construct<QuerySample_vec>("QS")(alloc_QS);
                ...
                {
                    std::cerr << "Met exception while sending Samples/IssueQuery to " << u_ << " : "
                              << ex.what() << std::endl;
                    assert(false);
                }
            }
    ...
    else if(m_scenario == "singlestream")
    {
        ...
        try
        {
            ...
            QuerySample_vec* QS_vec_data =
                m_c2s_shmem_map[tgt_mig_uuid]->construct<QuerySample_vec>("QS")(alloc_QS);
            for(auto& s_ : samples)
                QS_vec_data->push_back(s_);
        ...
        {
            std::cerr << "Met exception while sending Samples/IssueQuery to " << tgt_mig_uuid
                      << " : " << ex.what() << std::endl;
            assert(false);
        }
        moveServerTurn();
