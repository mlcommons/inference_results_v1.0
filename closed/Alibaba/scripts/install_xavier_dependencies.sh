...
sudo apt-get install -y virtualenv moreutils libnuma-dev numactl
...
# # Install CUDNN 8.1.0.77
# # TODO: This will be part of JetPack release
cd /tmp \
 && wget http://cuda-repo/release-candidates/Libraries/cuDNN/v8.1/8.1.0.77_20210124_29535611/10.2.89-r440/Lib/cudnn-10.2-linux-aarch64-v8.1.0.77.tgz \
 && tar -xvzf cudnn-10.2-linux-aarch64-v8.1.0.77.tgz \
 && rm cudnn-10.2-linux-aarch64-v8.1.0.77.tgz \
 && sudo rm -rfv /usr/lib/aarch64-linux-gnu/libcudnn* \
 && sudo cp -v cuda/include/cudnn* /usr/include/aarch64-linux-gnu/ \
 && sudo cp -v cuda/lib64/*so* /usr/lib/aarch64-linux-gnu/ \
 && rm -rf cuda

# Install TRT 7.2.3.2
# TODO: This will be part of JetPack release
cd /tmp \
 && sudo rm -rfv /usr/lib/aarch64-linux-gnu/libnvcaffe* \
 && sudo rm -rfv /usr/lib/aarch64-linux-gnu/libnvinfer* \
 && sudo rm -rfv /usr/lib/aarch64-linux-gnu/libnvonnxparser* \
 && sudo rm -rfv /usr/lib/aarch64-linux-gnu/libnvparser* \
 && wget http://cuda-repo/release-candidates/Libraries/TensorRT/v7.2/7.2.3.2-f75da003/10.2-r440/l4t-aarch64/tar/TensorRT-7.2.3.2.Ubuntu-18.04.aarch64-gnu.cuda-10.2.cudnn8.0.tar.gz -O TRT.tar.gz \
 && tar -xvzf TRT.tar.gz --wildcards TensorRT-7.2.3.2/targets/aarch64-linux-gnu/lib/*so* \
  TensorRT-7.2.3.2/targets/aarch64-linux-gnu/bin/* \
  TensorRT-7.2.3.2/include/* \
  TensorRT-7.2.3.2/python/tensorrt-7.2.3.2-cp36-none-linux_aarch64.whl \
  TensorRT-7.2.3.2/uff/* \
  TensorRT-7.2.3.2/graphsurgeon/* \
 && rm -f TRT.tar.gz \
 && sudo cp -rv TensorRT-7.2.3.2/targets/aarch64-linux-gnu/bin/* /usr/bin/ \
 && sudo cp -rv TensorRT-7.2.3.2/targets/aarch64-linux-gnu/lib/* /usr/lib/aarch64-linux-gnu/ \
 && sudo cp -rv TensorRT-7.2.3.2/include/* /usr/include/aarch64-linux-gnu/ \
 && sudo python3 -m pip install --no-deps --force-reinstall TensorRT-7.2.3.2/uff/*.whl TensorRT-7.2.3.2/graphsurgeon/*.whl TensorRT-7.2.3.2/python/tensorrt-7.2.3.2-cp36-none-linux_aarch64.whl \
 && rm -rf TensorRT-7.2.3.2
