...
class ConfigGrid:
    ...
    def __iter__(self):
        ...
        for term_dict in named_terms:
            ...
            # Maybe TODO:?
            # If we wanted to do arbitrary search logic (greedy search), we can do:
            #   1. get_stats_from_summary of the most recent run to get stats
            #   2. Store those results, and use them to find an appropriate next item.
            # Thus, the simple for loop would change into something like (using py3.8's assignment expressions)
            # while term := _get_next_term_using_internal_logic():
            #     yield term
            #     _update_internal_decision_making_state(term)
...
if __name__ == "__main__":
    ...
    run_group.add_argument("--norun", action='store_true', help="Skip running (run artifacts are already present)")
