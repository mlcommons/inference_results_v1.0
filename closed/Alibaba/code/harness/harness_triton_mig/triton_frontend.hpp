...
public:
    Server_SUT(std::string name, std::string model_repo_path, std::string model_name, uint32_t model_version,
        ...
        sharedMQ_ptr<mlperf::QuerySampleResponse> s2c_resp_mq, sharedMSM_ptr c2s_shmem, sharedMR_ptr s2c_shmem)
        : m_Name(name)
...
public:
    SUTShim(std::string name, std::string scenario, uint32_t batch_size, uint64_t timeout,
        std::vector<std::string>& mig_uuids,
        sharedMap_ptr<IPCcomm>& c2s_state_mq_map, sharedMap_ptr<IPCcomm>& s2c_state_mq_map,
    ...
    void ServerQueueHandler(std::string mig_uuid,
