    ...
    triton_frontend::sharedBAMap_ptr s2c_baddr_map)
{
    ...
    auto sutshim = std::make_shared<triton_frontend::SUTShim>("Triton_MultiMigServer", 
        scenario, FLAGS_batch_size, FLAGS_deque_timeout_us,
        mig_uuids, c2s_state_mq_map, s2c_state_mq_map, s2c_resp_mq_map, c2s_shmem_map, s2c_shmem_map);
...
std::vector<std::string> get_mig_uuids()
{
    ...
    std::unique_ptr<FILE, decltype(&pclose)> pipe(popen("nvidia-smi -L", "r"), pclose);
    if (!pipe)
    {
    ...
    LOG(INFO) << "nvidia-smi -L returned: " << std::endl << result;
...
int main(int argc, char** argv)
{
    ...
    case mlperf::TestScenario::MultiStream: expected_qps = test_settings.multi_stream_target_qps; break;
    ...
    if (FLAGS_mig_uuid == "N/A")
    {
        ...
        for (auto& u_ : mig_uuids)
        {
            ...
            LOG(INFO) << "Launching Inference process on " << u_;

            int c_pid = fork();

            // c_pid == 0 means this is forked child process
            // new cmd: org_cmdline + " --mig_uuid=" + it, so that MIG instance can be
            // captured
            if (c_pid == 0)
            {
        ...
        runClient(test_settings, log_settings, mig_lib, expected_qps, mig_uuids, c2s_state_mq_map, s2c_state_mq_map,
            s2c_resp_mq_map, c2s_shmem_map, s2c_shmem_map, s2c_baddr_map);
    }

    // Server process in MIG instances
    else
    {
        LOG(INFO) << "Performing inference on mig_uuid: " << FLAGS_mig_uuid;
        ...
        std::string c2s_shm_qstr = boost::replace_all_copy<std::string>(FLAGS_mig_uuid + "_C2S_SHM", "/", "_");
        triton_frontend::sharedMSM_ptr c2s_shmem
            = std::make_shared<bip::managed_shared_memory>(bip::open_only, c2s_shm_qstr.c_str());

        std::string s2c_shm_qstr = boost::replace_all_copy<std::string>(FLAGS_mig_uuid + "_S2C_SHM", "/", "_");
        bip::shared_memory_object s2c_shm(bip::open_only, s2c_shm_qstr.c_str(), bip::read_write);
        triton_frontend::sharedMR_ptr s2c_shmem = std::make_shared<bip::mapped_region>(s2c_shm, bip::read_write);

        // Run Server
        triton_frontend::ServerSUTPtr_t sut;

        runServer(test_settings, log_settings, sut, lib, expected_qps, c2s_state_mq, s2c_state_mq, s2c_resp_mq,
            c2s_shmem, s2c_shmem);
    }
