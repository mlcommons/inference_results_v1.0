...
DEVICE_FUNCTION void to_float(float (&dst)[4*N], int32_t (&src)[N], float scale = 1.f) {
    ...
    for( int i = 0; i < N; ++i ) {
        ...
        #pragma unroll
        for (int ii = 0; ii < 4; ++ii) {
            dst[4*i+ii] = (DO_SCALE)? __int2float_rn((int32_t)packed.x[ii]) * scale : __int2float_rn((int32_t)packed.x[ii]);
        }
    }
}
...
write_to_smem(int32_t *smem, int idx, const int (&x)[2]) {
...
template <int THREADS_PER_CTA>
static inline __device__ void
parallel_sums_8x4(float *smem, float (&x)[4], int nhw) {
    // The size of a warp.
    const int THREADS_PER_WARP = 32;
    // The number of warps in a CTA.
    const int WARPS_PER_CTA = THREADS_PER_CTA / THREADS_PER_WARP;
...
struct ParallelSums<8, 4> {
    template <int THREADS_PER_CTA>
    static inline __device__ void
    dispatch(float *smem, float (&x)[4], int nhw) {
