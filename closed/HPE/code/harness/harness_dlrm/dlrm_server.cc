...
void DLRMServer::IssueQuery(const std::vector<mlperf::QuerySample>& samples)
{
    ...
    if (mStartFromDevice)
    {
        ...
        std::transform(samples.begin(), samples.end(), numPairs.begin(), [&] (const mlperf::QuerySample& x) { return mQsls[0]->GetNumUserItemPairs(x.index); } );
    ...
    else
    {
        ...
        const int64_t splitThreshold{500};
        if (UseNuma() && numSamplesTotal > splitThreshold)
        {
            ...
            auto issueQueryOneBatchMaker = [](std::shared_ptr<BatchMaker> batchMaker,
                const std::vector<mlperf::QuerySample>& samples, const int64_t offset, const int64_t size)
            {
                batchMaker->IssueQuery(samples, offset, size);
            };
            std::vector<std::thread> issueThreads;

            int64_t offset{0};
            for (int64_t numaIdx = 0; numaIdx < numBatchMakers; numaIdx++)
            {
                ...
                issueThreads.emplace_back(issueQueryOneBatchMaker, mBatchMakers[numaIdx], std::ref(samples), offset, size);
                bindThreadToCpus(issueThreads.back(), mNumaConfig[numaIdx].second);
                offset += size;
            }
...
void DLRMServer::ProcessTasksFromDevice(std::shared_ptr<DLRMCore> dlrmCore, int deviceId, int profileIdx)
{
    ...
    while(true)
    {
        ...
        std::transform(tasks.begin(), tasks.end(), ebBundle->numericInputPtrBuf.GetHostPtr(),
            [&] (const DLRMTask& x) { return reinterpret_cast<DLRMNumericInputType *>(mQsls[0]->GetSampleAddress(x.querySample.index, 0, 0, deviceId)); } );
        NVTX_RANGE_POP();
        ...
        std::transform(tasks.begin(), tasks.end(), ebBundle->categoricalInputPtrBuf.GetHostPtr(),
            [&] (const DLRMTask& x) { return reinterpret_cast<DLRMCategoricalInputType *>(mQsls[0]->GetSampleAddress(x.querySample.index, 1, 0, deviceId)); } );
        NVTX_RANGE_POP();
