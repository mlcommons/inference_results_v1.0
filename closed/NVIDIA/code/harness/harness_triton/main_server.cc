...
void doInference()
{
    ...
    {
        sut->Init(FLAGS_min_sample_size, FLAGS_max_sample_size, FLAGS_buffer_manager_thread_count, FLAGS_batch_triton_requests, FLAGS_check_contiguity);
    }
    else
    {
        sut->Init(1, 1, FLAGS_buffer_manager_thread_count, FLAGS_batch_triton_requests, FLAGS_check_contiguity);
    }
    sut->ModelMetadata();
    ...
    // When batch_triton_requests is set in Offline scenario, warmup is quite long.
    if (!FLAGS_batch_triton_requests)
    {
        sut->Warmup(FLAGS_warmup_duration, expected_qps);    
    }
    else
    {
        std::cout << "Batch_triton_requests=true. Skipping warmup" << std::endl;
    }
