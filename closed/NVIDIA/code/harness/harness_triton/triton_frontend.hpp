...
public:
    ...
    void Init(size_t min_sample_size = 1, size_t max_sample_size = 1, size_t buffer_manager_thread_count = 0,
        bool batch_triton_requests = false, bool check_contiguity = false);
    void ModelMetadata();
    void ModelStats();
    ...
    void BatchCompletion(TRITONSERVER_InferenceResponse* response, const BatchResponseMetaData* response_metadata);
    void Done();
...
private:
    ...
    void IssueOrQueueCoalescedDLRMQuery(const std::vector<mlperf::QuerySample>& samples, size_t batch_start_sample,
        size_t batch_end_sample, size_t num_user_item_pairs, const std::vector<size_t>& batchNumPairs);
    void IssueCurrentBatchedRequest(RequestPool::Block* request_block, TRITONSERVER_InferenceTrace* trace);
