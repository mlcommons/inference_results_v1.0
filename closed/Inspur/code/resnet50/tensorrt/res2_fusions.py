...
def fuse_serial_3_conv2dc_onnx(registry, network):
    pattern1 = [{"name":"input",  "type":trt.ITensor,               "children":["add", "c_br2a"], "channels":512},
...
def fuse_res2_onnx(registry, network):
    ...
    match["input"] = network.get_layer(3).get_output(0)
    match["res2a"] = network.get_layer(11).get_output(0)
    match["res2b"] = network.get_layer(18).get_output(0)
    match["output"] = network.get_layer(25).get_output(0)
