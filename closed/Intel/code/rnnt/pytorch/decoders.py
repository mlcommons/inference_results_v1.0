...
class ScriptGreedyDecoder(torch.nn.Module):
    ...
    @torch.jit.export
    def forward_single_batch(self, x: torch.Tensor, out_lens: torch.Tensor, use_ipex, rank) -> Tuple[torch.Tensor, torch.Tensor, List[List[int]]]:
        ...
        # ipex does not help single step very much, go to non-ipex path
        if use_ipex:
            logits = logits.to(torch.device('cpu'))
            logits_lens = logits_lens.to(torch.device('cpu'))

        # inseq: TxBxF
        logitlen = logits_lens[0]
        sentence = self._greedy_decode(logits, logitlen)

        return logits, logits_lens, [sentence]
    ...
    def forward(self, x: torch.Tensor, out_lens: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, List[List[int]]]:
        ...
        for batch_idx in range(logits.size(0)):
            inseq = logits[batch_idx, :, :].unsqueeze(0)
            # inseq: TxBxF
    ...
    @torch.jit.export
    def forward_batch(self, x: torch.Tensor, out_lens: torch.Tensor, use_ipex, rank) -> Tuple[torch.Tensor, torch.Tensor, List[List[int]]]:
        """Returns a list of sentences given an input batch.
        ...
        return logits, logits_lens, sentences
    ...
    def _greedy_decode_batch(self, x: torch.Tensor, out_lens: torch.Tensor) -> List[int]:
        ...
        label_tensor = torch.tensor([self._SOS for i in range(batch_size*max_len*self._max_symbols_per_step)], dtype=torch.int64).view(batch_size, max_len*self._max_symbols_per_step)
        ...
        while True:
            ...
            time_idxs += symbols_added.ge(self._max_symbols_per_step)
            blankness.logical_or(symbols_added.ge(self._max_symbols_per_step))
            ...
            if self.count_nonzero(blankness) > 0:
                ...
                dim = f.size()[2]
                f = x.gather(2, fetch_time_idxs.unsqueeze(1).repeat(1, dim).unsqueeze(1).unsqueeze(1))[:, :, 0, :]
