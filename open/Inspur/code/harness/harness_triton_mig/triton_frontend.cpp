...
{
    m_finished = false;
    m_lck = std::unique_lock<std::mutex>(m_mtx, std::defer_lock);
    ...
    while (!is_finished())
    {
        ...
        if (!samples.empty())
        {
            try
            {
                ...
                QuerySample_vec* QS_vec_data = m_c2s_shmem_map[mig_uuid]->find_or_construct<QuerySample_vec>(sv_name.c_str())(alloc_QS);
                for (auto& s_ : samples)
...
void SUTShim::IssueQuery(const std::vector<mlperf::QuerySample>& samples)
{
        ...
        for (auto& u_ : m_mig_uuids)
        {
            if (samples_map[u_]->size() > 0)
            {
                try
                {
                    ...
                    QuerySample_vec* QS_vec_data = m_c2s_shmem_map[u_]->construct<QuerySample_vec>(sv_name.c_str())(alloc_QS);
    ...
    else if (policy == IssuePolicy::Bulk)
    {
        ...
        try
        {
            ...
            QuerySample_vec* QS_vec_data = m_c2s_shmem_map[tgt_mig_uuid]->find_or_construct<QuerySample_vec>(sv_name.c_str())(alloc_QS);
            for (auto& s_ : samples)
                QS_vec_data->push_back(s_);
