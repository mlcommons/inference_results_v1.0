...
void ResponseComplete(TRITONSERVER_InferenceResponse* response, const uint32_t flags, void* userp)
{
    ...
    if(response_metadata->m_TracePtr != nullptr)
    {
        ...
        sut->m_TraceManager->CaptureTimestamp(trace_id, TRITONSERVER_TRACE_LEVEL_MIN,
                                              "MLPerf Request Response RECV", TIMESPEC_TO_NANOS(ts));
    }
                         ...
                         const std::vector<std::string>& outputs)
{
    for (size_t count = 0; count < 2 /* REQUEST_POOL_COUNT */; count ++) {
        instances_.emplace_back(new RequestPool(initial_element_count, server, server_sut, model_name,
...
RequestPool::RequestPool(const size_t initial_element_count, TRITONSERVER_Server* server,
                         ...
                         const uint32_t model_version, const std::vector<InputMetaData>& inputs,
                         const std::vector<std::string>& outputs)
    : m_Head(nullptr), m_ReleasedHead(nullptr), m_Blocks(initial_element_count), m_Server(server), m_ServerSUT(server_sut),
...
void Server_SUT::Init(size_t min_sample_size, size_t max_sample_size, size_t buffer_manager_thread_count)
{
    ...
    FAIL_IF_ERR(TRITONSERVER_ServerOptionsSetPinnedMemoryPoolByteSize(server_options, ((uint64_t)1) << 29 /* 512 MB */),
